...
/*
*  Adds the parameter object to the num_items of the Stack. That is, the
...
void Stack<T>::Push(const T& item) {
  ...
  if(Size() == Capacity()){
    //maybe move this back up if issues...
...
/*
*  Removes the object on num_items of the Stack, and returns it. That is, remove
...
*  or DEFAULTCAPACITY.
*
*  RETURN: the element that used to be at the num_items of the Stack.
*/
template <class T>
T Stack<T>::Pop() {
  T top_item = items[num_items];
  items[num_items]= NULL;
  
  num_items--;
 
  double ratio = (double) 1.0/SHRINKRATE;
 
  double item = (double) (num_items+1)/Capacity();
...
/*
*  Finds the object on num_items of the Stack, and returns it to the caller.
...
* 
*  RETURN: the element at the num_items of the Stack.
*/
template <class T>
T Stack<T>::Peek() {
